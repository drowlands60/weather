...
object Weather extends IOApp.Simple {
    ...
    IO.ref(true).flatMap { continue => 
      ( for {
        _ <- weatherFunc
        repeat <- getInput("Do you want to check another city? (y/n): ")
        _ <- continue.set(repeat.toLowerCase == "y")
      } yield()).whileM_(continue.get)
    }
  ...
  def getWeather(city: Json): IO[String] = 
      city.hcursor.get[String]("Key") match {
        case Left(error) => IO.raiseError(new Exception(s"Failed to get location key: $error"))
        case Right(locationKey) =>
          val weatherURI: String =  s"http://dataservice.accuweather.com/currentconditions/v1/$locationKey?apikey=iUxLgSJkOao2GLy68sqFnV9G62sxMpEh"
  ...
  def printWeather(weatherJson: String, city: Json): IO[Unit] =
      parser.parse(weatherJson) match {
        case Right (json: Json) =>
          ...
          val temp_val: Either[io.circe.DecodingFailure, Double] = json.hcursor.downArray.downField("Temperature").downField("Metric").get[Double]("Value")
          val weather: Either[io.circe.DecodingFailure, String] = json.hcursor.downArray.get[String]("WeatherText")
          val daytime: Either[io.circe.DecodingFailure, Boolean] = json.hcursor.downArray.get[Boolean]("IsDayTime")
          temp_val match {
            case Right(value) => weather_string = weather_string + s"Temperature: $value"
            case _ => IO.unit
          }
          weather match {
            case Right(value) => weather_string = weather_string + s"\nWeather: $value"
            case _ => IO.unit
          }
          daytime match {
  ...
  def weatherFunc: IO[Unit] = {
    val uri = "http://dataservice.accuweather.com/locations/v1/topcities/50?apikey=iUxLgSJkOao2GLy68sqFnV9G62sxMpEh"
    for {
      _ <- IO.println("Weather app starting...")
      body <- EmberClientBuilder.default[IO].build.use { client => 
        ...
        case Right(response: Json) => 
          ...
          for {
            ...
            _ <- cityIndexOpt match {
              case Some(index) =>
                indexedCities.collectFirst  { case (json, i) if i == index => json } match {
                  case Some(cityJson) => 
                    for {
                      weatherBody <- getWeather(cityJson)
                      _ <- printWeather(weatherBody, cityJson)
                    } yield ()
